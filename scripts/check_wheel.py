...
def check_wheel(path: Path, version: str, platform: str | None) -> None:
    ...
    with ZipFile(path) as archive:
        ...
        data = f"rvx-{version}.data"
        web = f"{data}/data/share/rvx/web"
        required = {
            "rvx/__init__.py", "rvx/source.py", "rvx/adapters.py",
            "rvx/service.py", "rvx/config.py", "rvx/errors.py", "rvx/tracker.py",
            "rvx/_tracker_span.py", "rvx/_tracker_values.py",
            f"{data}/scripts/rvx", f"{data}/scripts/rvxd",
            f"{web}/index.html", f"{web}/login/index.html",
        ...
        if any(name in names for name in ("rvx/cli.py", "rvx/tui.py", "rvx/_server.py")):
            raise ValueError("Python runtime must contain only SDK adapters, not CLI/TUI launchers")
        ...
        for executable in (f"{data}/scripts/rvx", f"{data}/scripts/rvxd"):
            binary = archive.getinfo(executable)
            if not stat.S_IMODE(binary.external_attr >> 16) & 0o111:
                raise ValueError(f"bundled {PurePosixPath(executable).name} is not executable")
        html = archive.read(f"{web}/index.html").decode("utf-8")
        assets = re.findall(r'(?:src|href)="(/assets/[^"]+)"', html)
        if not assets or any(web + asset not in names for asset in assets):
            raise ValueError("bundled UI references missing assets")
        login = archive.read(f"{web}/login/index.html").decode("utf-8")
        login_assets = re.findall(r'(?:src|href)="(/login/assets/[^"]+)"', login)
        if not login_assets or any(web + asset not in names for asset in login_assets):
            raise ValueError("bundled login UI references missing assets")
        if platform and platform.startswith("manylinux_2_28_"):
            check_glibc(archive.read(f"{data}/scripts/rvx"), "rvx")
            check_glibc(archive.read(f"{data}/scripts/rvxd"), "rvxd")
    print(f"Complete RVX wheel: {path.name}")


def check_glibc(binary: bytes, name: str = "rvxd") -> None:
    ...
    if not versions or max(versions) > (2, 28):
        raise ValueError(f"{name} exceeds the manylinux_2_28 glibc contract: {versions}")
